...
export default function BDIWorkRepor() {
  const {
    openSidebar,
  ...
  const [tlName, setTLName] = React.useState("Select TL");
  const [tlId, setTLId] = React.useState();
  ...
  // //! handle search bar select change
  const handleSearchBarSelectChange = (e) => {
    setTLId(e.target.value);
    if (tlData) {
      for (let i = 0; i < tlData.length; i++) {
        if (tlData[i]._id === tlId) {
          setTLName(tlData[i].name);
        }
      }
    }
    console.log(tlName);
  ...
  // //! handle diplay data (take 4 args: role of the authors, id of the creator of the author, startdate, enddate)
  const handleDisplayData = async (e) => {
    ...
    if (tlId === "" || tlId === null) {
      toast.error("Plese select any TL.");
      return;
    }
    setLoading(true);
                  ...
                  <form onSubmit={handleDisplayData}>
                    <div>
                      <select
                        ...
                        name=""
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 m-2"
                        onChange={(e) => handleSearchBarSelectChange(e)}
                      >
