...
export default function AllAllocatedTaskToUs() {
  const {
    openSidebar, setOpenSidebar, // for opening and closing sidebar in big devices 
    profileData,
    viewAllocatedTaskVivaDateAndPageInfo,
  ...
  const handleFetchData = async () => {
    ...
    setLoading(true);
    const res = await viewAllocatedTaskVivaDateAndPageInfo(
      profileData._id,
      startDate,
      endDate,
      pageNo,
          ...
          <tr key={ind}>
            ...
            <td>
              <select
                ...
                value={d.status}
                onChange={(e) =>
                  handleStatusUpdateOfTheAllocatedTask(e.target.value, d._id)
                }
              >
  ...
  // //! handle status update of the allocated task
  const handleStatusUpdateOfTheAllocatedTask = async (value, taskDBId) => {
    const res = await handleFieldValueUpdateWithoutCreatorId(
      "status",
      value,
      taskDBId
    );
    ...
    {openSidebar && <ProvideSidebar />}
      ...
      <div className="page-comp w-full min-w-0 flex-auto lg:static lg:max-h-full lg:overflow-visible">
        <div className="container">
          <div className="heading">Allocated Task's</div>
          {loading ? (
