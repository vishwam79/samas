...
export default function ShowAllMD() {
  ...
  const fetchAllData = async () => {
    setLoading(true);
    const res = await fetchCommonRolePerson("MD");
      ...
      data.map((d, ind) => {
        return (
          <tr key={ind}>
            ...
            {/* <td><button type='button' className={d.status === 'active' ? 'activeStatusButton' : 'blockedStatusButton'} onClick={(e)=>handleBlockUnBlock(ind, d._id, d.status==='active'?'blocked':'active')}>{d.status === 'active'?'Block':'Unblock'}</button></td> */}
          </tr>
        );
      })
    );
    ...
    const filteredData =
      ...
      copyOfOriginalData.filter(
        (item) =>
          ...
          (item.role && item.role.toLowerCase().includes(text.toLowerCase()))
      );
      ...
      <div className="global-container lg:flex">
        ...
        <div className="page-comp w-full min-w-0 flex-auto lg:static lg:max-h-full lg:overflow-visible">
          <div className="comp-container1">
            <div className="heading">ALL MD's</div>
            {loading ? (
